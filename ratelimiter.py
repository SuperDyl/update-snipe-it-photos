...
class RateLimiter:
    ...
    random_sleep: Callable[['RateLimiter'], None]

    def _random_wait(self) -> None:
        minimum: float = (self.time_limit / self.request_limit) / 2
    ...
    def __init__(self,
                 ...
                 random_sleep: Optional[Callable[[None], None]] = None):
    ...
    def _combine_url(self, added_url: str) -> str:
        return self.api_url if not added_url else r'/'.join((self.api_url, added_url))

    def _request_preprocessing(self) -> None:
        if not self._wait():
            self.random_sleep(self)
        self._add_request()
